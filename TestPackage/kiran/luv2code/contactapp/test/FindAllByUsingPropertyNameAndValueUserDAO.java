...
public class FindAllByUsingPropertyNameAndValueUserDAO {
	
	public static void main(String[] args) {
		ApplicationContext ctx = new AnnotationConfigApplicationContext(SpringRootConfig.class);
		UserDAO userDAO = ctx.getBean(UserDAO.class);
		List<User> user1 = userDAO.findByProperty("phone", "78677666632");
		for(User user : user1) {
			System.out.println("User Details: "+user.getName() +"=="+user.getLogin_name() +"=="+ user.getEmail() +"=="+ user.getPhone());
		}
		System.out.println("Data By Find All By Property and Value.");
	}
}
